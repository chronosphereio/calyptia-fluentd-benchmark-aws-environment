...
rate_2000_calyptia = pd.read_csv(os.path.join(base_path, 'usage-calyptia-fluentd-2000.tsv'), sep='\t', na_values='.')
rate_2000_td = pd.read_csv(os.path.join(base_path, 'usage-td-agent-2000.tsv'), sep='\t', na_values='.')
rate_5000_calyptia = pd.read_csv(os.path.join(base_path, 'usage-calyptia-fluentd-5000.tsv'), sep='\t', na_values='.')
rate_5000_td = pd.read_csv(os.path.join(base_path, 'usage-td-agent-5000.tsv'), sep='\t', na_values='.')
...
df_td = pd.DataFrame({
    ...
    "2000 lines/sec w/ td-agent": rate_2000_td[resource_key_format.format("td-agent".title())],
    "5000 lines/sec w/ td-agent": rate_5000_td[resource_key_format.format("td-agent".title())],
})
if divide_base > 1:
...
df_calyptia = pd.DataFrame({
    ...
    "2000 lines/sec w/ calyptia-fluentd": rate_2000_calyptia[resource_key_format.format("calyptia-fluentd"[:15].title())],
    "5000 lines/sec w/ calyptia-fluentd": rate_5000_calyptia[resource_key_format.format("calyptia-fluentd"[:15].title())],
})
if divide_base > 1:
...
palette2 = sns.color_palette('pastel', n_colors=5)
